...
export class ExecucaoAtividadeService extends GenericService {
  ...
  getAll(): Observable<ExecucaoAtividade> {
    return this.http.get<ExecucaoAtividade>(
      this.getUrlApi() + this.getEndpointExecucaoAtividade(),
  ...
  getById(id: number) {
    return this.http.get<ExecucaoAtividade>(
      this.getUrlApi() + this.getEndpointExecucaoAtividade() + '/' + id,
      { headers: this.getHeadersWithUserAuthorization() }
    );
  }

  save(execucaoAtividade: ExecucaoAtividade) {
    return this.http.post<ExecucaoAtividade>(
      this.getUrlApi() + this.getEndpointExecucaoAtividade(),
      execucaoAtividade,
      { headers: this.getHeadersWithUserAuthorization() }
    );
  ...
  update(execucaoAtividade: ExecucaoAtividade) {
    return this.http.put<ExecucaoAtividade>(
      this.getUrlApi() + this.getEndpointExecucaoAtividade(),
      execucaoAtividade,
      { headers: this.getHeadersWithUserAuthorization() }
    );
